...
export default function Optimize() {
  ...
  useEffect(() => {
    setLoading(true);
    api.getOptimize(undefined, undefined, selectedState).then(d => {
      setData(d);
      setLoading(false);
    }).catch(() => setLoading(false));
  }, [selectedState]);

  if (loading) return <div className="loading"><div className="loading-spinner"></div><p>Running RL optimizer for {stateName}...</p></div>;
    ...
    <div className="fade-in">
      <div className="page-header" style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <h1 style={{ margin: 0 }}>🧠 {stateName} RL Optimizer</h1>
        </div>
      </div>
